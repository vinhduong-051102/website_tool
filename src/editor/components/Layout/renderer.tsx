...
export const Renderer = ({
  ...
  return (
    <Layout
      hasSider={Boolean(node.props.hasSider)}
      style={{
        ...resolvedStyles,
        minHeight: isEmpty && !isPreviewMode ? "150px" : undefined,
      }}
    ...
    >
      {isEmpty && !isPreviewMode && (
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none text-gray-500 text-[10px] font-mono select-none">
          {isOver ? "Release to drop region!" : "Layout (Drop Header, Sider, Content here)"}
        </div>
      )}
      {children}
    </Layout>
