...
export const Renderer = ({
  ...
  return (
    <Card
      title={node.props.title ? String(node.props.title) : undefined}
      bordered={node.props.bordered !== false}
      hoverable={Boolean(node.props.hoverable)}
      size={node.props.size as any}
      loading={Boolean(node.props.loading)}
      style={{
        ...resolvedStyles,
        minHeight: isEmpty && !isPreviewMode ? "100px" : undefined,
      }}
    ...
    >
      {isEmpty && !isPreviewMode && (
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none text-gray-500 text-[10px] font-mono select-none">
          {isOver ? "Release to drop component!" : "Card Content (Drop here)"}
        </div>
      )}
      {children}
    </Card>
