...
export const Renderer = ({
  ...
  return (
    <Flex
      vertical={resolvedStyles.flexDirection === "column"}
      justify={resolvedStyles.justifyContent as any}
      align={resolvedStyles.alignItems as any}
      gap={resolvedStyles.gap as any}
      style={{
        ...resolvedStyles,
    ...
    >
      {isEmpty && !isPreviewMode && (
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none text-gray-500 text-[10px] font-mono select-none">
          {isOver ? "Release to drop component!" : "Container (Drop here)"}
        </div>
      )}
      {children}
    </Flex>
